...
processor = Processor(data_file_path + "rsr_product_template.csv", delimiter=",")
...
product_template_mapping = {
    ...
    ,"map_price":mapper.val( "map_price")
    ...
    ,"expanded_product_description": mapper.val("expanded_product_description")
    ,"website_description": mapper.val("description_sale")
    ...
    ,"is_storable": mapper.val("is_storable")
}
...
processor.process(product_template_mapping, data_file_path + "rsr.product.template.csv", {"model": "product.template", "context": "{'tracking_disable': True}", "worker": 4, "batch_size": 20})
processor.process(product_supplierinfo_mapping, data_file_path + "rsr.product.supplierinfo.csv", {"model": "product.supplierinfo", "context": "{'tracking_disable': True}", "worker": 4, "batch_size": 20}, "set")
processor.write_to_file(data_file_path + "rsr_product_template.sh", python_exe='python3', path='')